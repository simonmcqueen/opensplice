...
SET TOUCHSTONE_DIR=%TOUCHSTONE%\c
SET TOUCHSTONE_C_SOURCE=%TOUCHSTONE% metrics alive_writer
SET GEN_DIR=%GEN_BASE_DIR%\c
SET GEN_INCLUDES=/I"%GEN_DIR%"
REM SET VENDOR_INCLUDES=%VENDOR_INCLUDES% /I"%VENDOR_DIR%"
SET LINKOBJS=
FOR %%f IN (%OS_SOURCE% %GENERATED_C_SOURCE% %TOUCHSTONE_C_SOURCE%) DO SET LINKOBJS=!LINKOBJS! %OBJ_DIR%\%%f.obj
...
FOR %%f IN (%TOUCHSTONE_C_SOURCE%) DO %C_COMPILER% /wd4996 /nologo %VENDOR_INCLUDES% %GEN_INCLUDES% %DDS_C_INCLUDES% %OS_INCLUDES% /c "%TOUCHSTONE_DIR%\%%f.c" /Fo%OBJ_DIR%\%%f.obj

IF NOT EXIST %BIN_DIR% MD %BIN_DIR%
FOR %%f in (%TOUCHSTONE%) DO %C_LINKER% /nologo %LINKOBJS% %OS_LDLIBS% %DDS_C_LDLIBS% /OUT:%BIN_DIR%\%%f_c.exe
